...
//Prototipos designados al menu
void menu();
void sub_menu1();
void sub_menu6();

//Prototipos designados para la implementación
...
//Prototipos desingados para la metodologia
...
int main(){  
  	...
  	//proceso de validacion de periodo
  	system("COLOR F8");
	...
	if(archivo){
    	...
    	do{
    		...
    		cout << "\n El proyecto dispone de una capacidad de [1-3] meses:" << endl;
...
void promediosABCreal(vector<promedioABC>& promediosParaABC, producto* p1, int periodo, int& totalDeVentas){
	...
	while(!feof(f)){
		...
		//refrescar acumuladores
		promedioP = 0;
		promedioV = 0;
		valor_de_utilizacion = 0;

		// refresco y avanzo los iteradores
		y=0;
		j++;
...
void metodoAgrupacionPareto(int totalDeVentas, vector<promedioABC> promediosParaABC) {
    ...
    sizeGrupoA = totalDeVentas * 0.2;           //50   330 ventas totales
    sizeGrupoB = totalDeVentas * 0.3;           //66
    sizeGrupoC = totalDeVentas - sizeGrupoA - sizeGrupoB;     //214
